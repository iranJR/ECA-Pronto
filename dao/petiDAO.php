...
class petiDAO
{
    ...
    public function salvar($peti)
    {
        ...
        try{
            if ($peti->getIdPeti() != "") {
                $statement = $pdo->prepare("UPDATE tb_peti SET str_month=:str_month, 
 str_year=:str_year, db_value=:db_value, str_situation=:str_situation, tb_beneficiaries_id_beneficiaries=:tb_beneficiaries_id_beneficiaries, 
 tb_city_id_city=:tb_city_id_city WHERE id_peti = :id;");
                $statement->bindValue(":id", $peti->getIdPeti());
            } else{
                $statement = $pdo->prepare("INSERT INTO tb_peti (str_month, str_year, db_value, str_situation, tb_beneficiaries_id_beneficiaries, tb_city_id_city ) VALUES (:str_month, :str_year, :db_value, :str_situation, :tb_beneficiaries_id_beneficiaries, :tb_city_id_city)");
            }
            ...
            $statement->bindValue(":str_situation", $peti->getStrSituation());
    ...
    public function atualizar($peti)
    {
        global $pdo;
        try {
            $statement = $pdo->prepare("SELECT id_peti, str_month, str_year, db_value, str_situation, tb_beneficiaries_id_beneficiaries, tb_city_id_city FROM tb_peti WHERE id_peti = :id");
            $statement->bindValue(":id", $peti->getIdPeti());
            if ($statement->execute()) {
                ...
                $peti->setStrSituation($rs->str_situation);
    ...
    public function tabelapaginada()
    {
        ...
        $sql = "SELECT id_peti, str_month, str_year, db_value, str_situation, tb_beneficiaries_id_beneficiaries, tb_city_id_city FROM tb_peti LIMIT {$linha_inicial}, " . QTDE_REGISTROS;
        ...
        if (!empty($dados)):
            ...
            <tbody>";
            foreach ($dados as $petii):
                echo "<tr>
                    <td style='text-align: center'>$petii->id_peti</td> 
                    <td style='text-align: center'>$petii->str_month</td> 
                    <td style='text-align: center'>$petii->str_year</td> 
                    <td style='text-align: center'>$petii->db_value</td>
                    <td style='text-align: center'>$petii->str_situation</td>  
                    <td style='text-align: center'>$petii->tb_beneficiaries_id_beneficiaries</td> 
                    <td style='text-align: center'>$petii->tb_city_id_city</td> 
                    <td style='text-align: center'><a href='?act=upd&id=$petii->id_peti' title='Alterar'><i class='ti-reload'></i></a></td>
                    <td style='text-align: center'><a href='?act=del&id=$petii->id_peti' title='Remover'><i class='ti-close'></i></a></td>
                    </tr>";
            endforeach;
            echo "
              </tbody> 
